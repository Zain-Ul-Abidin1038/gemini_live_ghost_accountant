...
3. THE System SHALL use `GeminiSchemas.financeCommand` as the Response_Schema for all financial parsing
4. WHEN Gemini_Service returns a response, THE System SHALL receive structured data instead of text
...
4. WHEN parsing fails, THE System SHALL return a clear error message from the Gemini_Service
...
2. THE System SHALL include the original error message from Gemini_Service
...
### Requirement 7: Integration with Gemini Service V3

**User Story:** As a developer, I want seamless integration with GeminiServiceV3, so that structured parsing works with the existing AI infrastructure.

#### Acceptance Criteria

1. THE System SHALL use `geminiService.sendStructuredMessage()` for all financial parsing
...
4. THE System SHALL handle both successful and failed responses from Gemini_Service
...
### Compatibility
- Works with existing GeminiServiceV3 infrastructure
