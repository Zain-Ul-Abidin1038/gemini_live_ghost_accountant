...
The Marathon Agent is an autonomous expense prediction system that monitors user location and proactively suggests expenses when the user remains stationary at commercial locations. It leverages Gemini 3 Pro for intelligent expense prediction, displays reasoning through the Ghost Trace interface, and validates expenses against current tax laws before persisting to AWS DynamoDB.

## Glossary

- **Marathon_Agent**: The autonomous background service that monitors location and triggers expense predictions
- **Gemini_3**: The Gemini 3 Pro AI model used for expense prediction and logic auditing
...
- **Logic_Audit**: A validation process performed by Gemini 3 to verify expense compliance with tax regulations
...
**User Story:** As a user, I want Gemini 3 to predict likely expenses based on my location and context, so that I can quickly log expenses without manual entry.

#### Acceptance Criteria

1. WHEN a Stationary_Event at a Commercial_Coordinate is detected, THE Marathon_Agent SHALL invoke Gemini_3 with location context, business type, time of day, and user expense history
2. WHEN Gemini_3 receives a prediction request, THE Gemini_3 SHALL generate an Expense_Prediction including amount estimate, category, deduction percentage, and confidence score
...
4. IF Gemini_3 cannot generate a confident prediction, THEN THE Marathon_Agent SHALL log the event without user notification
5. THE Gemini_3 SHALL include reasoning steps in the prediction response for Ghost_Trace display
...
2. WHEN displaying reasoning, THE Ghost_Trace SHALL show each reasoning step from Gemini_3 in chronological order with neon teal (#00F2FF) accents
...
1. WHEN the user accepts an Expense_Prediction, THE Safe_Layer SHALL invoke Gemini_3 to perform a Logic_Audit
2. WHEN performing a Logic_Audit, THE Gemini_3 SHALL validate the expense against 2026 tax law rules including deduction percentages, category compliance, and documentation requirements
...
4. WHEN invoking Gemini 3, THE Marathon_Agent SHALL use the existing firebase_vertexai integration
