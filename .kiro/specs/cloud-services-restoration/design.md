...
## Overview
This document outlines the design for restoring cloud AI and backend services to Ghost Accountant, including Gemini 3 Pro AI integration and AWS Amplify services (Cognito, S3).
...
5. AI analysis with context
   └─> GeminiService.analyzeReceiptImage()
...
```json
{
  ...
  "geminiModel": "gemini-3-pro-vision",
  "version": "1.0",
...
**Purpose:** Interface with Google Gemini 3 Pro API for AI analysis.

**API Methods:**

1. **`analyzeReceiptImage({base64Image, memoryContext, region})`**
   - Sends image + context to Gemini Vision API
...
```dart
// Development
const geminiApiKey = String.fromEnvironment('GEMINI_API_KEY_DEV');
const awsRegion = 'us-east-1';
const s3Bucket = 'ghost-accountant-dev';

// Production
const geminiApiKey = String.fromEnvironment('GEMINI_API_KEY_PROD');
const awsRegion = 'us-east-1';
