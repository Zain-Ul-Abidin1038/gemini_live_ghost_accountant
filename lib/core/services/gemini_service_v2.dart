...
final geminiServiceV2Provider = Provider((ref) => GeminiServiceV2());

/// Gemini 3 Service with Production-Grade Optimizations
...
class GeminiServiceV2 {
  ...
  // Base URL for Gemini 3 Flash Preview
  ...
  Future<Map<String, dynamic>> sendStructuredMessage({
    ...
    try {
      ...
      safePrint('[Gemini V2] Exception: $e');
      return {
        'success': false,
        'error': e.toString(),
      };
...
Return confidence score (0-1) for OCR accuracy.''';
    ...
    try {
      ...
      safePrint('[Gemini V2 Vision] Exception: $e');
    ...
    for (int i = 0; i < delays.length; i++) {
      try {
        ...
        if (response.statusCode != 503) {
          safePrint('[Gemini V2] Error ${response.statusCode}: ${response.body}');
          return response;
        }

        // 503 - retry with backoff
        safePrint('[Gemini V2] 503 error, retrying in ${delays[i].inMilliseconds}ms...');
  ...
  String? _extractThoughtSignature(Map<String, dynamic> response) {
    try {
      ...
      safePrint('[Gemini V2] Failed to extract thought signature: $e');
    }
    return null;
  }
