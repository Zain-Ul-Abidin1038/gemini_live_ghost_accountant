...
import 'gemini_memory.dart';
import 'gemini_logger.dart';
import 'gemini_router.dart';
import 'gemini_cost_estimator.dart';
import 'gemini_fallback.dart';
import 'gemini_validator.dart';
import 'gemini_parser.dart';

final geminiServiceV3Provider = Provider((ref) => GeminiServiceV3(
      memory: ref.read(geminiMemoryProvider),
      logger: ref.read(geminiLoggerProvider),
      router: ref.read(geminiRouterProvider),
      costEstimator: ref.read(geminiCostEstimatorProvider),
      fallback: ref.read(geminiFallbackProvider),
      validator: ref.read(geminiValidatorProvider),
    ));

/// Gemini Service V3 - Production-Grade Layered AI System
/// 
/// Architecture:
/// UI → SimpleChatService → GeminiService V3
///   ├── Request Builder
...
class GeminiServiceV3 {
  final GeminiMemory memory;
  final GeminiLogger logger;
  final GeminiRouter router;
  final GeminiCostEstimator costEstimator;
  final GeminiFallbackEngine fallback;
  final GeminiResponseValidator validator;
  ...
  static String get _apiKey {
    ...
    try {
      ...
      safePrint('[Gemini] dotenv not loaded: $e');
    }
    ...
    if (fallbackKey.isEmpty || fallbackKey == 'AIzaSyA10HbjmIeRuQ_1CxV7cZrfEeb5JXn91Ms') {
      safePrint('[Gemini] Using fallback API key');
  ...
  Future<Map<String, dynamic>> sendMessage({
    ...
    try {
      ...
      memory.lastThoughtSignature = GeminiParser.extractThoughtSignature(response);
      ...
      // Extract text
      final result = GeminiParser.extractText(response);
  ...
  Future<Map<String, dynamic>> sendStructuredMessage({
    ...
    try {
      ...
      final data = GeminiParser.extractJson(response);
  ...
  Future<Map<String, dynamic>> analyzeReceiptImage({
    ...
    try {
      ...
      String systemInstruction = '''You are Ghost Accountant AI specialized in receipt analysis.
...
Return confidence score (0-1) for OCR accuracy.''';
      ...
      final receiptData = GeminiParser.extractJson(response);
  ...
  Future<Map<String, dynamic>> _postWithRetry(
    ...
    for (int i = 0; i < delays.length; i++) {
      try {
        ...
        if (response.statusCode == 429 || (response.statusCode >= 400 && response.statusCode < 500)) {
          safePrint('[Gemini V3] Error ${response.statusCode}: ${response.body}');
    ...
    throw Exception('Gemini API unavailable after retries');
  }
