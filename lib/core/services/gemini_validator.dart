...
final geminiValidatorProvider = Provider((ref) => GeminiResponseValidator());
...
class GeminiResponseValidator {
  void validateText(Map<String, dynamic> response) {
    ...
    if (content == null) {
      throw Exception('Invalid Gemini response: no content');
    }
    
    final parts = content['parts'];
    if (parts == null || (parts as List).isEmpty) {
      throw Exception('Invalid Gemini response: no parts');
    }
  }
