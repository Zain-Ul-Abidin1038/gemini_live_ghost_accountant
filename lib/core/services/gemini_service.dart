...
/// Gemini 3 REST Service with Thinking Config
/// Uses GCP API key to access Gemini 3 with thinking capabilities
class GeminiService {
  // Gemini API Key from GCP
  ...
  // Using Gemini 3 Flash Preview with GCP API key
  // Model: gemini-3-flash-preview (faster, optimized for speed)
  ...
  /// Send message to Gemini with high-level thinking
  ...
  Future<Map<String, String>> sendMessage({
    ...
    try {
      ...
      String fullSystemInstruction = systemInstruction ?? 
          'You are a Ghost Accountant AI assistant specialized in tax deductions and financial analysis.';
      ...
      // Construct request body for Gemini with thinking mode
      ...
      safePrint('[Gemini API] Sending request...');
      ...
      if (response.statusCode == 200) {
        ...
        if (candidates != null && candidates.isNotEmpty) {
          ...
          safePrint('[Gemini API] Response received');
          
          return {
            'text': textOutput.toString(),
            'thoughtSignature': 'gemini-response',
          };
        }
      } else {
        safePrint('[Gemini API] Error: ${response.statusCode} - ${response.body}');
        return {
      ...
      safePrint('[Gemini API] Exception: $e');
      return {
        'text': 'Failed to connect to Gemini: $e',
        'thoughtSignature': 'error',
      };
    }
  ...
  Future<Map<String, dynamic>> analyzeReceiptImage({
    ...
    try {
      ...
      String systemInstruction = '''You are a Ghost Accountant AI specialized in receipt analysis and tax deductions.
...
Return JSON format:
{
  ...
  "thoughtSummary": "brief explanation of deduction logic"
      ...
      safePrint('[Gemini Vision] Analyzing receipt...');
      ...
      if (response.statusCode == 200) {
        ...
        if (candidates != null && candidates.isNotEmpty) {
          ...
          if (textOutput != null) {
            ...
            try {
              ...
              receiptData['thoughtSignature'] = 'gemini-vision';
              return receiptData;
            } catch (e) {
              safePrint('[Gemini Vision] JSON parse error: $e');
              return {
                'vendor': 'Unknown',
                'total': 0.0,
                'tax': 0.0,
        ...
        safePrint('[Gemini Vision] Error: ${response.statusCode} - ${response.body}');
      }
    } catch (e) {
      safePrint('[Gemini Vision] Exception: $e');
    }
  ...
  Future<String?> sendRawMessage(String prompt) async {
    try {
      ...
      safePrint('[Gemini API] Sending RAW request...');
