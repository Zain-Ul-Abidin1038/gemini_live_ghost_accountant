...
import 'package:ghost_accountant/features/ghost_navigator/domain/navigation_task.dart';
...
class GhostNavigatorService {
  ...
  Stream<Map<String, dynamic>> executeTask(NavigationTask task) async* {
    safePrint('[Ghost Navigator] Starting task: ${task.description}');
    ...
    try {
      ...
      safePrint('[Ghost Navigator] Error: $e');
  ...
  Future<Map<String, dynamic>> _planTask(String taskDescription) async {
    final prompt = '''You are Ghost Navigator - an AI agent that can navigate apps and websites like a human.
...
Return ONLY valid JSON.''';
    ...
    try {
      ...
      safePrint('[Ghost Navigator] Planning error: $e');
      rethrow;
    }
  }
  ...
  Future<Map<String, dynamic>> _executeStep(Map<String, dynamic> step, String taskDescription) async {
    ...
    safePrint('[Ghost Navigator] Executing: $description');
...
If no clear amount, estimate based on typical costs.''';
    ...
    try {
      ...
      safePrint('[Ghost Navigator] Transaction extraction error: $e');
    }
  ...
  Future<void> _recordTransaction(Map<String, dynamic> transaction) async {
    try {
      ...
      safePrint('[Ghost Navigator] Transaction recorded: ₹$amount');
    } catch (e) {
      safePrint('[Ghost Navigator] Failed to record transaction: $e');
    }
  }
  ...
  Future<ScreenAnalysis> analyzeScreen(File screenshot) async {
    try {
      ...
      await geminiService.analyzeReceiptImage(
      ...
      safePrint('[Ghost Navigator] Screen analysis error: $e');
      rethrow;
    }
  }
}
