...
import 'package:ghost_accountant/core/services/gemini_service_v3.dart';
import 'package:amplify_flutter/amplify_flutter.dart';
import 'package:ghost_accountant/features/finance/services/unified_finance_service.dart';
import 'package:ghost_accountant/features/finance/domain/income_entry.dart';
import 'package:ghost_accountant/features/finance/domain/expense_entry.dart';
import 'package:ghost_accountant/features/finance/domain/ledger_entry.dart';

final aiFinanceParserProvider = Provider((ref) => AIFinanceParser(
      geminiService: ref.read(geminiServiceV3Provider),
      financeService: ref.read(unifiedFinanceServiceProvider),
    ));

class AIFinanceParser {
  final GeminiServiceV3 geminiService;
  ...
  Future<Map<String, dynamic>> _executeAction(Map<String, dynamic> parsed) async {
    ...
    try {
      switch (action) {
        ...
        case 'unknown':
        default:
          // For non-financial messages, use Gemini for general conversation
...
''';
  ...
  Future<Map<String, dynamic>> _handleGeneralConversation(
    ...
    try {
      // Use Gemini V3 for conversational response (auto-selects Flash)
      final response = await geminiService.sendMessage(
        prompt: originalMessage,
        systemInstruction: '''You are Ghost Accountant, a friendly AI financial assistant.
...
Examples:
- "hi" → "Hello! I'm Ghost Accountant, your AI financial assistant. I can help you track expenses, scan receipts, manage loans, and provide financial insights. How can I assist you today?"
...
''',
      ...
      return {
        'success': true,
        'message': 'Hello! I\'m Ghost Accountant, your AI financial assistant. I can help you track expenses, scan receipts, and manage your finances. Try saying "I spent \$50 on lunch" or "show my balance".',
        'thoughtSignature': '',
      };
    }
  }
  ...
  Future<Map<String, dynamic>> _handleMultipleTransactions(String userMessage) async {
    ...
    // Use Gemini to split the message into individual transactions
...
IMPORTANT: Return ONLY the JSON array with transaction strings, no other text or explanation.''';
    ...
    try {
      final splitResponse = await geminiService.sendMessage(
        prompt: splitPrompt,
...
Return ONLY the JSON object, no other text.''';
    ...
    try {
      final response = await geminiService.sendStructuredMessage(
        prompt: prompt,
