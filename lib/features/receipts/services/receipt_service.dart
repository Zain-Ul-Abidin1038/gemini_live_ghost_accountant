...
import 'package:ghost_accountant/core/services/audit_vault_service.dart';
import 'package:geolocator/geolocator.dart';

final receiptServiceProvider = Provider((ref) => ReceiptService(ref));

/// Receipt Service with Gemini API (GCP) + AWS Memory Pipeline
class ReceiptService {
  ...
  /// 3. Call Gemini 3 Pro with memory context
  ...
  Future<Receipt> analyzeReceipt(File imageFile) async {
    ...
    // Step 5: Call Gemini API (GCP) with memory context
    traceService.addTrace("[Ghost Agent] 🤖 Analyzing with Gemini API...");
    final analysisResult = await geminiService.analyzeReceiptImage(
